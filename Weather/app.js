...
for (let i = 0; i < 100; i++) {
  if (i === 0 || i === 1) {
    ...
    iconsDay[i] = '<img src = images/fog.png';
  } else if (i >= 51 && i <= 57) {
    iconsDay[i] = '<img src = images/rainy.png';
  } else if ((i >= 61 && i <= 67) || (i >= 80 && i <= 82)) {
    iconsDay[i] = '<img src = images/rainy.png';
  } else if ((i >= 71 && i <= 77) || i === 85 || i === 86) {
    iconsDay[i] = '<img src = images/snow.png';
  } else if (i >= 95 && i <= 99) {
...
async function checkWeatherDaily(longitude, latitude,sunset,sunrise){
  ...
  var numIcons = document.getElementsByClassName('iconDaily');
  console.log(data);
  ...
  for(i = 0; i < numIcons.length; i++){
    ...
    if(today.getHours() < sunset && today.getHours() > sunrise){
      numIcons[i].innerHTML = iconsNights[data.daily.weathercode[i+1]];
    } else{
      numIcons[i].innerHTML = iconsDay[data.daily.weathercode[i+1]];
    }
  }
}
